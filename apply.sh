...
original_author="ammarisme"
original_name="cq_backend_flask"
original_urlname="cq-backend-flask"
original_description="Awesome cq_backend_flask created by ammarisme" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/cq_backend_flask "${name}"
...
then
    ...
    echo "then run 'cq_backend_flask' or 'python -m cq_backend_flask'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'cq_backend_flask' or 'python -m cq_backend_flask' or 'poetry run cq_backend_flask'"
fi
