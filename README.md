...
- 🎯 Entry points to execute your program using `python -m <cq_backend_flask>` or `$ cq_backend_flask` with basic CLI argument parsing.
...
---
# cq_backend_flask Flask Application

Awesome cq_backend_flask created by ammarisme
...
```bash
git clone https://github.com/ammarisme/cq-backend-flask cq_backend_flask
cd cq_backend_flask
make install
```
...
```bash
cq_backend_flask create-db   # run once
cq_backend_flask populate-db  # run once (optional)
cq_backend_flask add-user -u admin -p 1234  # ads a user
cq_backend_flask run
```
